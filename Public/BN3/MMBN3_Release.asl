...
split
{
		...
		if (settings[element.Attribute("id").Value])
		{
			if (element.Attribute("check") != null && element.Attribute("value") != null)
			{
				...
				switch(check)
				{
					...
					case "BattleRewardGet":
						{
							...
							if (vars.Helper["BattleReward"].Changed && vars.Helper["BattleReward"].Current == value)
							{
					...
					case "FiresStarted":
						{
							if (vars.Helper["Progress"].Current == 85 && vars.Helper["MainArea"].Current == 0 
								&& vars.Helper["SubArea"].Current == 3 && vars.Helper["GameState"].Changed 
								&& vars.Helper["GameState"].Current == 12)
								{
	...
	// Check Completed
	if (vars.Helper["Progress"].Current == 122 && vars.Helper["MainArea"].Current == 131 && vars.Helper["SubArea"].Current == 4)
	{
		return vars.Helper["FinalScene"].Old == 0 && vars.Helper["FinalScene"].Current == 1);
	}
}
